...
def upload_instagram(conn, c, user_id, user_input, image_url, caption):
    ...
    post_data = {"post": caption, "mediaUrls": [image_url], "platforms": ["instagram"]}

    response = requests.post(
        "https://app.ayrshare.com/api/post", json=post_data, headers=headers
    )
    ...
    if response.status_code == 200:
        ...
        print(
            f"Error posting image to Instagram: {response.status_code} - {response.text}"
        )
